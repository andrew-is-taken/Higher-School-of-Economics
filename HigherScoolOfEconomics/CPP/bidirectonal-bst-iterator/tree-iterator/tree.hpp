...
struct tree_node {
    ...
    tree_node(tree_node &&a, int val, std::nullptr_t)
        : value(val), left(std::make_unique<tree_node>(std::move(a))) {
        left->up = this;
    }
    tree_node(std::nullptr_t, int val, tree_node &&b)
        : value(val), right(std::make_unique<tree_node>(std::move(b))) {
        right->up = this;
    }
...
struct tree {
    ...
    struct iterator {
        // Для того, чтобы реализовать end(), вам скорее всего понадобится ещё
        // одно поле
        tree_node *p;
        tree_node *root;
